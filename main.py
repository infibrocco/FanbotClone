...
async def wiki(ctx, *args):
  try:
    ...
    if len(args) < 1:
      ...
      dic = {'titles' : [['https://fancade.com'+str(link['href']) for link in links],
...
async def _help(ctx):
  try:
    ...
    embed.add_field(name=f'{PREFIX}ping', value='Returns ping', inline=False)
    embed.add_field(name=f'{PREFIX}search', value='Returns results by searching from fancade.com/search', inline=False)
    embed.add_field(name=f'{PREFIX}wiki or {PREFIX}doc', value='Returns results by searching from fancade.com/wiki', inline=False)
